...
| `OLLAMA_NUM_PARALLEL` | `8` | Number of parallel inference threads. M4 Pro has 10 CPU cores; ~8 for optimal throughput. |
...
**QUICK_MODEL: `qwen-quick`**
- Built from `modfile-qwen-quick` (base: `qwen2.5-coder:3b`)
- Optimized for speed on small tasks; ~2GB RAM, <1s response time
- Use for: single-file edits, quick syntax questions, brief code snippets
...
|----------|---------|-------------|
| `DEFAULT_MODELS` | `nomic-embed-text,qwen2.5-coder:14b` | Models to ensure at startup (comma-separated) |
...
| `OLLAMA_NUM_PARALLEL` | `8` | Parallel inference threads (M4 Pro has 10 cores) |
...
- **Context size**: `num_ctx=16384` set in modfile for efficient code reviews.

## Performance Expectations

- **qwen-devops** (14b): ~40 tokens/sec, ~300ms/token latency
- **qwen-quick** (3b): ~100 tokens/sec, <100ms response time
...
1. Edit `.env`: change `DEFAULT_MODELS` and create corresponding `modfiles/` entries
...
Reduce `DEFAULT_MODELS` to smaller models; `qwen2.5-coder:7b` is available if 14B doesn't fit.
