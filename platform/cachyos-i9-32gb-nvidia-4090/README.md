...
|-------|-------------|-----------------|-----------|--------------|
| `qwen3-coder:30b-gpu` (~30B) | ~60 | 50 | ~22 GB | 10 layers |
| `gemma4:26b-devops` | ~52 | 50 | ~20 GB | ~2 layers |
| `devstral-small-2-gpu` | ~32 | 32 (all) | ~4 GB | None |
...
**Tuning guide:**
- RTX 4090 24GB: Use `OLLAMA_GPU_LAYERS=50` for 30B models (leaves 2GB headroom)
- RTX 3090 24GB: Same settings work
- RTX 4080 16GB: Reduce to `OLLAMA_GPU_LAYERS=30` for 30B models, or use 14B models
...
| `OLLAMA_MODELS` | `/home/ollama/models` | Directory for model weights (must be on partition with ~60GB free per 30B model) |
...
| `DEFAULT_MODELS` | `qwen3-coder:30b-gpu,nomic-embed-text:latest` | Models to ensure at startup |
| `DEVOPS_MODEL` | `gemma4:26b-devops` | Custom DevOps model for heavy tasks |
| `QUICK_MODEL` | `devstral-small-2-gpu` | Fast small model for quick queries |
| `QDRANT_PORT` | `6333` | Qdrant HTTP API |
| `QDRANT_GRPC_PORT` | `6334` | Qdrant gRPC API |

## Models

### GPU-Optimized 30B Coder
- **Modfile**: `qwen3-coder-30b-gpu.modelfile`
- **Base**: `qwen3-coder:30b` (~18GB download)
- **GPU layers**: 50 of ~60 → ~22 GB VRAM + ~2 GB RAM
- **Performance**: ~80 tokens/sec, ~100ms/token latency
- **Use**: Heavy coding tasks, complex reasoning

### Gemma4 DevOps Model (26B)
- **Modfile**: `gemma4-26b-devops.modelfile`
- **Base**: `gemma4:26b` (~16GB download)
- **GPU layers**: 50 of ~52 → ~20 GB VRAM
- **Performance**: ~70 tokens/sec
- **Use**: Enterprise DevOps, security-focused tasks

### Devstral Small (Fast 2-GPU)
- **Modfile**: `devstral-small-2-gpu.modelfile`
- **Base**: `devstral-small-2` (~2GB download)
- **GPU layers**: all 32 → ~4 GB VRAM
- **Performance**: ~150 tokens/sec, ~50ms/token
- **Use**: Quick queries, fast responses
...
- **devstral-small-2-gpu (2B)**: 150+ tok/s (fully GPU-resident → extremely fast)
- **30B models**: 60–80 tok/s (partial GPU, partial CPU)
- **26B models**: 70–100 tok/s (partial GPU)
...
Ensure internet connectivity; large models (30B = ~18GB) require stable connection.
